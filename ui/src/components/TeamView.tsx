        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
            {people.map((p, i) => {
              ...
              const isUnassigned = p.name === 'Nepřiřazeno'

              return (
                <div
                  key={p.name}
                  onClick={() => toggle(p.name)}
                  className={`bg-white rounded-2xl border-2 cursor-pointer transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${
                    isOpen ? 'border-emerald-400 shadow-lg shadow-emerald-50' : 'border-slate-200'
                  }`}
                >
                  {/* Avatar */}
                  <div className="flex flex-col items-center pt-5 pb-2 px-3">
                    {isUnassigned ? (
                      <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center text-slate-300 mb-1">
                        <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0" />
                        </svg>
                      </div>
                    ) : (
                      <PixarAvatar palette={palette} size={80} seed={i} />
                    )}
                    <div className={`text-sm font-bold mt-1 text-center truncate w-full ${isUnassigned ? 'text-slate-400 italic' : 'text-slate-800'}`}>
                      {p.name}
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center pb-1">
                    <PieSlice pct={p.pct} color={p.color} />
                  </div>
                  ...
                  <div className="px-4 pb-4 text-center">
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">
                      {p.pct.toFixed(1)} % rozpočtu · {p.items.length} pol.
                    </div>
                    {p.spent > 0 && (
                      <div className="text-[10px] text-violet-500 mt-0.5 font-medium">
                        čerpáno {fmtFull(p.spent)} Kč
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div key={p.name} ref={el => { detailRefs.current[p.name] = el }} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    {p.items.length} položek · {fmtFull(p.budget)} Kč
                  </p>
                </div>
                <button
                  onClick={() => toggle(p.name)}
